...
extension Reactive where Base: CMAltimeter {
    static public func manager(createAltimeter: @escaping () throws -> CMAltimeter = { CMAltimeter() }) -> Observable<Altimeter> {
        return Observable.create { observer in
            do {
                let altimeter = try createAltimeter()
                observer.on(.next(Altimeter(altimeter: altimeter)))
            }
            catch let e {
                observer.on(.error(e))
            }
...
extension Reactive where Base: CMAltimeter {
    public var altimeterData: Observable<CMAltitudeData> {
        return memoize(key: &accelerationKey) {
            Observable.create { observer in
                let altimeter = self.base
                let operationQueue = OperationQueue()
                operationQueue.maxConcurrentOperationCount = 1

                altimeter.startRelativeAltitudeUpdates(to: operationQueue, withHandler: { (data: CMAltitudeData?, error: Error?) -> Void in
                    guard let data = data else {
                        return
                    }
                    observer.on(.next(data))
...
public struct Altimeter {

    public let altimeterObservable: Observable<CMAltitudeData>?

    init(altimeter: CMAltimeter) {
        if CMAltimeter.isRelativeAltitudeAvailable() {
            altimeterObservable = altimeter.rx.altimeterData
        } else {
            altimeterObservable = nil
        }
    }
